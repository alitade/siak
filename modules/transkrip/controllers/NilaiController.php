...
class NilaiController extends ModController{
	...
	public function actionCetakAll(){
		...
		if(is_array($chk)){
			//$chk="'".implode("','",$chk)."'";
			$TABLE="";
			$ModMhs = \app\models\Mahasiswa::find()->where(['mhs_nim'=>$chk])->all();
			$n=0;
    ...
    public function actionPindah($id){
        ...
        $acc=false;
        if($sub){
            foreach($sub['jurusan'] as $k=>$v){if($v==$ModMhs->jr_id){$acc=true;}}
	...
	public function actionDokNil($id){
        ...
        $acc=false;
        if($sub){
            foreach($sub['jurusan'] as $k=>$v){if($v==$ModJdwl->bn->kln->jr_id){$acc=true;}}
            if($acc==false){ throw new NotFoundHttpException('The requested page does not exist.');}
        }
        #HAK Akses
