...
class WisudaController extends ModController{
    ...
    public function actionTranskrip($id){
        $model 	= $this->findModel($id);
		$ModMhs = \app\models\Mahasiswa::find()->where(" mhs_nim=:n and jr_id in(select jr_id from tbl_jurusan where jr_jenjang='s2')",[':n'=>$model->npm])->one();
		if(!$ModMhs){return $this->redirect(["/transkrips2/nilai/mhs"]);}
