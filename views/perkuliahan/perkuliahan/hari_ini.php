    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                ...
                'format'=>'raw',
                'value'=>function($model){
                    return Html::a($model[mhs],['dirit/peserta-kuliah','id'=>$model[ds_fid],'m'=>substr($model[jdwl_masuk],0,2)]);
                }
            ],
            [
                'label'=>'Absensi Dosen',
                'value'=>function($model){
                    return ($model[ds_masuk]?:"xx:xx")." - ".($model[ds_keluar]?:"xx:xx");
                }
            ],
            [
                ...
                'buttons' => [
                    'view'=> function ($url, $model, $key) {
                        return Html::a('<i class="fa fa-eye" ></i> Detail'
                            ,['perkuliahan/view-hari-ini','id' => $model['ds_fid'],'m'=>substr($model[jdwl_masuk],0,2)],['target'=>'_blank']);
                    },
                    'fp_awal'=> function ($url, $model, $key) {
                        return Html::a('<i class="glyphicon glyphicon-file"></i> Form Pulang Awal'
                            ,['trx-finger/form','id' => $model->tf_jdwl_id,'view'=>'t','t'=>'1'],['target'=>'_blank']);
                    },
                    'pAwal'=> function ($url, $model, $key) {
                        return Html::a('<i class="glyphicon glyphicon-eye-open"></i> Pulang Awal'
                            ,['rekap-absen/pulang-awal','id' => $model->tf_jdwl_id,'m'=>substr($model->tf_jdwl_masuk,0,2),'view'=>'t']);
                    },
