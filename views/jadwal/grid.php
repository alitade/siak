...
<br /><br />
	...
	echo $d= GridView::widget([
        ...
        'columns' => [
            ...
            [
                'format'=>'raw',
                'header'=>'<i class="fa fa-user-plus"></i>',
                'width'=>'5%',
                'value'=>function($model){
	                return $model->mhs?:0;
                    #return Html::a( ($model->mhs?:0),Yii::$app->urlManager->createUrl(['akademik/jdw-detail','id' => $model->jdwl_id]),['title' => Yii::t('yii', 'Detail'),]);
                },
			...
			[
				...
				'buttons' => [
					...
					'split'=> function ($url, $model, $key) {
						if(!Funct::acc('/bisa/ajr-split')){return false;}
						return Html::a(
