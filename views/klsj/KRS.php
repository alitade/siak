    ...
    <table class='table table-hover table-condensed'>
        <tr>
            ...
            <td style="vertical-align:middle">
                <?php 
				$dataKR=[];
				foreach(\app\models\Kurikulum::find()->where()->all() as $d){$dataKR[$d->kr_kode]=$d->kr_kode."-".$d->kr_nama;}
        ...
        <tr>
            <td>Nama</td>
            <td><?= $mhs->dft->bio->Nama;?></td>
        </tr>
        <tr>
            <td>Jurusan</td>
            <td><?= $mhs->jr->jr_jenjang."-".$mhs->jr->jr_nama;?></td>
        </tr>
        <tr>
            <td>Program Studi</td>
            <td><?= $mhs->pr->pr_nama;?></td>
        </tr>
        <tr>
            <td>Pembimbing</td>
            <td><?= $mhs->wali->ds_nm;?></td>
        </tr>        
        <?php endif;?>
    </table>
</div>
...
if($mhs){
	if(isset($_GET['Krs']['kurikulum']) ){
		...
		if($tahun!='NULL#NULL'){
		echo (!Funct::acc('/klsj/tambah-krs')?"":
			Html::a(
				"<i class='glyphicon glyphicon-plus glyphicon-white'></i> Tambah Matakuliah",
				['klsj/tambah-krs','Krs[kurikulum]' => $tahun,'nim'=>$_GET['nim']],
			 ...
			 [
				 ...
				 'value' => function($data) {
						return false ? "" : Html::a('<center><i class="glyphicon glyphicon-trash"></i></center>',[
                            "klsj/delete-krs","id"=>$data["krs_id"],"kurikulum" => $_GET["Krs"]["kurikulum"],"nim" => @$_GET["nim"],]
