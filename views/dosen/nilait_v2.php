...
$Columns =[
        ...
        [     
			...
			'value'		=>  function($data){
				...
				$abs =Yii::$app->db->createCommand(" SELECT dbo.persensiMhs($data[krs_id]) absen ")->queryOne();
				if($abs){return	number_format($abs['absen'],0)."%";}
				return "0%";
			}
        ], 
        [     
...
if($header['Lock']<32){
	$this->registerJs("$('#InputNilai').Tabledit({
		...
		columns: {
			...
			.(array_key_exists('10',Funct::TANGGAL($model->kln_uas)) ?"":"[6, 'krs_tgs2'],")
...
if($P==0||$G==0){
	echo '<div class="alert alert-danger"><h4>Bobot Nilai Masih Kosong, Klik '.Html::a('disini',['/dosen/bobot-nilai','id'=>$header['bn_id']],['class'=>'btn btn-primary']).' untuk mengatur bobot nilai</h4></div>';
}else{
	if($header['Lock']>=62){		
...
GridView::widget([     
    ...
    'summary' => "<div class='summary'>
					<div class='info'>Kelola Nilai Mahasiswa || $header[mtk_kode] $header[mtk_nama] <br>
                    $header[ds_nm]<br>
				  ...
				  Menampilkan {begin} sampai {end} dari {totalCount} Mahasiswa || Halaman {page} dari {pageCount} Halaman</div>
				  ",
    'columns' => $Columns,
    'responsive'=>false,
    'hover'=>true,
