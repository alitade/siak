...
class LaporanController extends Controller{
    ...
    public function actionKehadiranDosen(){
        ...
        $searchModel = new LAbsenDosenSearch;
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(),"isnull(parent,0)=0");
    ...
    public function actionKehadiranDosenDetail($id){
        $model=LAbsenDosen::findOne($id);

        $searchModel = new LAbsenDosenSearch;
        $dataProvider = $searchModel->search(Yii::$app->request->getQueryParams(),['parent'=>$model->id]);

        return $this->render('hadir_dosen_detail',[
            'dataProvider' => $dataProvider,
            'searchModel' => $searchModel,
            'model'=>$model,
        ]);
            ...
            if($cModel){
                ...
                if(isset($_POST['pr'])){
                    ...
                    $def=Yii::$app->db->createCommand(" 
                        ...
                        INNER JOIN m_absen_dosen ma on( ma.jdwl_id=jd.jdwl_id and isnull(ma.RStat,0)=0 and ma.kr_kode_='$model->kr_kode')
                ...
                if(isset($_POST['ex'])){
                    ...
                    if($model->save()){
                        ...
                        $sql="
                            insert into labsen_dosen_det(id_labsen,id_absen,ds_id,ds_tipe,ds_nm,ds_get_id,pelaksana,masuk,keluar,jdwl_id,jdwl_kls,jdwl_hari,jdwl_masuk,jdwl_keluar,mtk_kode,mtk_nama,mtk_sks,sesi,tgl_perkuliahan,tipe,ltipe,GKode_,totmhs,hadir)
                            SELECT $model->id,
                            ...
                            ,NULL tipe,0 ltipe,t.GKode_,jd._totmhs,iif(isnull(t.ds_stat,0)=1,1,0)
    ...
    public function actionKehadiranDosenView($id){
        ...
        if($model){
            ...
            $def=Yii::$app->db->createCommand(" select top 1 * from labsen_dosen_det where id_labsen=$model->id  ORDER BY ds_nm,jdwl_hari,jdwl_masuk, GKode_,jdwl_kls")->queryOne();
        ...
        if($model){
            ...
            $def=Yii::$app->db->createCommand(" 
                	SELECT  jd.GKode 
                            ...
                            INNER JOIN tbl_matkul mk on(mk.mtk_kode=bn.mtk_kode and ISNULL(mk.RStat,0)=0)
                            INNER JOIN tbl_dosen ds1 on(ds1.ds_id=isnull(t.ds_get_id,t.ds_id) and isnull(ds1.RStat,0)=0)
                            LEFT JOIN labsen_dosen_det ld on(ld.id_absen=t.id AND isnull(ld.hadir,0)=1)
                            WHERE kr_kode_='$model->kr_kode'
