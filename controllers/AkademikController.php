...
class AkademikController extends Controller
{
	...
	public function actionMtk(){
		...
		$data = new \app\controllers\matakuliah;
        return $this->render('mtk_index',$data->akademikmatakuliah());
	}

    public function actionMtkCreateV2(){
        $model = new Matkul;
	...
	public function actionAjrView($id,$pid=NULL){
		$B=false;
        $model_jadwal = new Jadwal;
        $model_jadwal->bn_id = $id;
        $ModBn = BobotNilai::findOne($id);
		$ViewForm="../akademik/schedule__form";


		#Penambahan SubJawal
		if(isset($pid)){
			$ViewForm="../akademik/schedule__form_child";
			if(is_numeric($pid)){
				...
				if(isset($_POST['subjdwl'])){
					if ($model_jadwal->load(Yii::$app->request->post())) {
						...
						if($countBentrok){
							...
							if($idParent){return $this->redirect(['ajr-view', 'id' => $id]);}
						}
					}
			...
			}else{return $this->redirect(['ajr-view','id'=>$id]);}
		}
		...
		$qDetJdwl_="
			SELECT
				jdw.jdwl_id,jdw.GKode,jdw.rg_kode,
				bn.id bn_id,ds.ds_id,jdw.jdwl_id,
				dbo.subJdwl(t1.id) jadwal
				,t1.*
				,ds.ds_nm
				,jdw.jdwl_hari,jdw.jdwl_kls,jdw.rg_kode
				,isnull(t.totMhs,0) totMhs
			from tbl_bobot_nilai bn
			INNER JOIN tbl_dosen ds on(ds.ds_id=bn.ds_nidn and (ds.RStat is null or ds.RStat='0'))
			INNER JOIN tbl_jadwal jdw on(jdw.bn_id=bn.id and (jdw.RStat is null or jdw.RStat='0'))
			INNER JOIN dbo.JadwalDosenBn($id) t1 on(t1.jdw=jdw.jdwl_id)
			LEFT JOIN (
				SELECT krs.jdwl_id, count(krs.jdwl_id) totMhs
				FROM tbl_krs krs
				INNER JOIN tbl_mahasiswa mhs on(mhs.mhs_nim=krs.mhs_nim and (mhs.RStat is null or mhs.RStat='0'))
				INNER JOIN keuanganfix.dbo.student s on( s.nim COLLATE Latin1_General_CI_AS=mhs.mhs_nim)
				INNER JOIN keuanganfix.dbo.people p ON(s.no_registrasi=p.no_registrasi)
				WHERE isnull(krs.RStat,0)=0
				and krs.krs_stat='1'
				AND krs.kr_kode_='".$ModBn->kln->kr_kode."'
				group by krs.jdwl_id
			) t on(t.jdwl_id=jdw.jdwl_id)
			order by t1.GKode, t1.h,t1.m,t1.k,t1.jenjang,t1.jurusan,t1.program
		";
		$qDetJdwl_=Yii::$app->db->createCommand($qDetJdwl_)->queryAll();
		*/
		#Req Data
        if ($model_jadwal->load(Yii::$app->request->post())) {
...
//
	...
	// Gkode(jadwla gabungan)
	public function actionJdwGabUpdate($id){
