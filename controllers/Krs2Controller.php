 ...
 */
class Krs2Controller extends Controller{
                            ...
                            if($model->krs_stat!=$_POST['app'][$model->jdwl_id]){
                                ...
                                if(!$dis){
                                    ...
                                    if( $model->save(false)){
                                        ...
                                        if($krsStat==2){
                                            $JD['JD'][$model->jdwl->jdwl_hari][]=['m'=>$model->jdwl->jdwl_masuk,'k'=>$model->jdwl->jdwl_keluar,];
                                            Yii::$app->db->createCommand("update tbl_jadwal set mhs+=1 WHERE  jdwl_id=$model->jdwl_id")->execute();
                                        }
                                        if($_POST['app'][$model->jdwl_id]==1){$ket="Menyetujui";}
    ...
    public function actionAdminJadwal($id){
        ...
        //$JD['K']=$kuota;
        #echo $sql = " exec menuKrsMhs '$mMhs->mhs_nim','$mKr->kr_kode'";
        return $this->render('/krs/admin/mhs_jdwl',[
            'data'=>$dataProvider,
            'mKr'=>$mKr,
            'maxSks'=>$maxSks,
            'listJadwal'=>$listJadwal,
    ...
    public function actionSplitPerkuliahan($id){
        ...
        $modelPilih=Jadwal::find()
            ...
            ->where("isnull(tbl_jadwal.RStat,0)=0 and tbl_jadwal.jdwl_id <> $id ")->all();
        return $this->render('split_perkuliahan',[
            'model'=>$model,
            'modelPilih'=>$modelPilih,
        ]);
    ...
    public function actionSplitmPerkuliahanProc($id,$split){
        ...
        if($_POST['selection']){
            $id=$_POST['selection'];
            Krs::updateAll(['jdwl_id'=>$modelPilih->jdwl_id],['jdwl_id'=>$model->jdwl_id,'mhs_nim'=>$id]);
    ...
    public function actionExProcs(){
        Funct::v($_POST);
        foreach ($_POST['selection'] as $k=>$v ){Yii::$app->db->createCommand("exec dbo.exportPerwalian $v")->execute();}
        return $this->redirect(Yii::$app->request->referrer?:Yii::$app->homeUrl);
    }
