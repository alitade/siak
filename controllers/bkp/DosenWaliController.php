...
class DosenWaliController extends Controller
{
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if($model->save()){
                Functdb::insLog($model::$ID,$model->jr_id.'|'.$model->ds_id,'C',
                    "Menadaftarkan ".$model->ds->bio->glr_depan.' '.$model->ds->bio->nama.$model->ds->bio->glr_belakang." (".$model->ds->ds_nidn.") sebagai dosen wali"
                );
    ...
    public function actionView($id1, $id){
        ...
        if(isset($_POST[u])){
            if(isset($_POST['selection'])){
                Mahasiswa::updateAll(['ds_wali'=>null,'uuid'=>Yii::$app->user->identity->id,'utgl'=>new  Expression("getdate()")],[
    ...
    public function actionMhsCreate($id,$id1){
        ...
        if(isset($_POST[u])){
            if(isset($_POST['selection'])){
                Mahasiswa::updateAll(['ds_wali'=>$model->ds_id,'uuid'=>Yii::$app->user->identity->id,'utgl'=>new  Expression("getdate()")],[
                    'mhs_nim'=>$_POST['selection']
                ]);
