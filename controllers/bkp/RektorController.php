...
class RektorController extends Controller
{
	...
	public function actionGrafikMhs(){
		//echo "kln ".$_POST['chart']['pr'];
		$sesi = 1;
		$kon =" 1, ' and 1=0' ";
		$Head='';
		if(isset($_POST['chart'])){
			
			$KlnId 	= (int)$_POST['chart']['pr'];
			$kr		= (int)$_POST['chart']['kr'];
			$ModKr	= Kurikulum::findOne($kr);
			$sesi 	= (int)$_POST['chart']['s'];
			@$model	= Kalender::findOne($KlnId);
			$Head='Data Kehadiran Perkuliahan Pertemuan Ke-'.$sesi.' Tahun Akademik '.$ModKr->kr_nama;
			$kon =" $sesi, 'and kln.kr_kode=''".$kr."'' ' ";
		}
		
		
		

		$sql = Yii::$app->db->createCommand(" EXEC rHadirJdw $kon ")	;
		$dataProvider= new ArrayDataProvider(['allModels' => $sql->queryAll(),]);
		$columns = [
		[
			'attribute'=>'Jadwal',
			'value'=>function($data){
				return implode('&nbsp;',explode(' ',$data['Jadwal']));
			},'format'=>'raw',
		],
/*		[
			'attribute'=>'Dosen',
			'value'=>function($data){
				return implode('&nbsp;',explode(' ',$data['Dosen']));
			},'format'=>'raw',
		],
*/		[
			'label'=>'Matakuliah',
			'value'=>function($data){
				return $data['Kode']." ".$data['Matakuliah'];
			},'format'=>'raw',
		],
		'Kls.','Mahasiswa','Target',
		[
		'label'=>"Sesi $sesi",
		'value'=>function($data)use($sesi){
			$d = explode('|',$data["Sesi ".$sesi]);
			$s ="Sesi $sesi";
			return $d[1];
		},'format'=>'raw',
		],
...
// ========================= drop ==============================
    ...
    public function actionJurusan() {
        ...
        if (isset($_POST['depdrop_parents'])) {
            $id = end($_POST['depdrop_parents']);
            $list = Jurusan::find()->select(" jr_id, concat(jr_jenjang,' ',jr_nama) jurusan")->distinct(true)
			->andWhere(['fk_id'=>$id])->asArray()->all();
            $selected  = null;
            if ($id != null && count($list) > 0) {
                $selected = '';
                foreach ($list as $i => $kota) {
					$kode=$kota['jr_id'];
                    $out[] = ['id' =>$kota['jr_id'],'name' => $kota['jurusan'] ];
                    if ($i == 0){$selected=$kota['jr_id'];}
                }
                // Shows how you can preselect a value
                echo Json::encode(['output' => $out, 'selected'=>$selected]);
                return;
    ...
    public function actionKlnfak() {
        $out = [];
        if (isset($_POST['depdrop_parents'])) {
            $id = end($_POST['depdrop_parents']);
            $list = Kalender::find()->select(' kr_kode, jr_id ')
			->innerJoin('tbl_jurusan jr','(jr.jr_id=tbl_kalende.jr_id)')
			->innerJoin('tbl_fakultas fk','(fk.fk_id=jr.fk_id)')
			->distinct(true)->andWhere(['kr_kode'=>$id])->asArray()->all();
            $selected  = null;
            if ($id != null && count($list) > 0) {
                $selected = '';
                foreach ($list as $i => $kota) {
