...
class KrsController extends Controller
{
    ...
    public function actionMhs(){
        ...
        $mKr    = Kalender::find()->where("
            isnull(RStat,0)=0
            and CAST(GETDATE() as date) BETWEEN kln_krs and cast(dateadd(day,-1,kln_masuk) as date) 
    ...
    public function actionTransfer(){
        $n=0;
        $model =TKrsHead::find()->where("isnull(RStat,0)=0 and isnull(tf,0)=0 and isnull(app,0)=1")->all();
        foreach ($model as $d){
            $statSave=0;
            if($d->tf!=1){
