...
*/	
	...
	private function view($user,$pass){
		...
		where (username='".trim($user)."' or username2='".trim($user)."' )
		and pass='".md5($pass)."'
		and username not in(
    ...
    private function getUser($username,$pass=''){
			...
			$que="
            Select *, CAST(id AS VARCHAR(200)) idku, lower(password) password, 
			lower(convert(varchar(32),HASHBYTES('MD5',('$password'+pass_kode+ CAST(tipe as VARCHAR(12)))),2)) authKey from user_
            where (username='$username' or username2='$username')
			 and lower(password) = lower(convert( varchar(32),HASHBYTES('MD5', ('$password'+pass_kode+ CAST(tipe as VARCHAR(12)))),2))
            ";
			*/
	...
	*/
    ...
    public function getAksesDet(){return $this->hasMany(SubAksesDet::className(),['user_id'=>'id']);}
